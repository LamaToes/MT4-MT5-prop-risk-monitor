                ...
                const int &spread[])
{
   ...
   for(int i=0; i<totalPositions; i++)
   {
      ...
      if(MathAbs(posSL - posPrice) < point * 2) // Within 2 pips = break even
      {
      ...
      if(posType == POSITION_TYPE_BUY && posSL > posPrice) isProfitLocked = true;
      if(posType == POSITION_TYPE_SELL && posSL < posPrice) isProfitLocked = true;
   ...
   {
      warnings += "TOO MANY POSITIONS ("+IntegerToString(positionCount)+") ";
      warnColor = clrRed;
   }
